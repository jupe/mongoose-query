...
var parseQuery = function(query){
  ...
  */
  ...
  {
    if( cond['$or'] ) {
      if( !conditions.hasOwnProperties('$or') ){
        conditions['$or'] = [];
      }
  ...
  var parseParam = function(key, val){
    ...
    if (val === "") {
      ...
      var parts = val.split(',');
      this.sort = {};
      this.sort[parts[0]] = parts.length > 1 ? parseInt(parts[1]) : 1;
    } else {
      if( convertToBoolean(val) != -1 ) {
        ...
        if( b == false ) {
          ...
          this.conditions[ '$or' ] = [] 
          this.conditions[ '$or' ].push(orCond);
          orCond[ lcKey ] = b;
          this.conditions[ '$or' ].push(orCond);
        }
        ...
        if (operator === "gt" ||
          ...
          condition[ key]['$elemMatch']['key']  = value[0];
          condition[ key]['$elemMatch']['value']  = value[1];
