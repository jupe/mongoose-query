...
var parseQuery = function (query, options) {
  ...
  var parseParam = function (key, val) {
    ...
    if (val === "") {
      ...
      if (typeof val === 'string' && convertToBoolean(val) != -1) {
        ...
        if (operator === "gt" ||
          ...
          operator === "nin" ||*/
          operator === "lte") {
          if (!_.isNumber(val) && isStringValidDate(val)) {
            val = new Date(val);
          }
          var tmp = {}
          ...
          qy.q[key] = {};
          qy.q[key]['$elemMatch'] = {};
          qy.q[key]['$elemMatch']['key'] = val[0];
          qy.q[key]['$elemMatch']['value'] = val[1];
          ...
          var parts = typeof val === 'string' ? val.split('|') : '';
          if (parts.length > 1) {
            var arr = []
  ...
  if (query.q) {
    qy.q = toJSON(decodeURIComponent(query.q));
    _.each(qy.q, walker);
  }
...
var doQuery = function (query, model, options, callback) {
  ...
  return executeQuery(q, model, options, callback);
};

function executeQuery(q, model, options, callback) {
  var find = model;
  switch (q.t) {
    case('find'):
    case('findOne'):
...
QueryPlugin.doQuery = doQuery;
QueryPlugin.executeQuery = executeQuery;

module.exports = exports = QueryPlugin;