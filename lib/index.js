...
function QueryPlugin(schema, options = {}) {
  ...
  const tmp = {};
  const doQuery = function doQuery(data, opt, callback) {
    logger.debug('doQuery:', data, opt, callback);
    const q = parseQuery(data, opt);
    if (!(this instanceof mongoose.Model)) {
      // @todo why this doesn't work?
      //throw new Error('Something weird happen - query didnt happen inside Model');
    }
  ...
  const query = function query(data, callback) {
  ...
  const leanQuery = function leanQuery(data, callback) {
    return doQuery.bind(this)(
      data,
