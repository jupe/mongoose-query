...
const {
  toBool, toNumber, escapeRegExp, isObjectID, parseDateCustom
} = tools;

function parseQuery(query, options = {}) {
  /**

  reserved keys: q,t,f,s,sk,l,p,to,x,d
  ...
  to=<maxTimeMS> - timeout for query
  x=<explain> - explain query
  d=<allowDiskUse> - allow disk usage for query
  ...
  */
  ...
  function parseParam(key, val) {
    ...
    if (val === '' && operator !== 'empty' && operator !== '!empty') {
      ...
      // key={i}<value>
      addCondition(lcKey, new RegExp(`^${escapeRegExp(val)}$`, 'i')); // http://scriptular.com/
    } else if (operator === 'e') {
      // key={e}<value>
      addCondition(lcKey, new RegExp(`${escapeRegExp(val)}$`));
    } else if (operator === 'b') {
      addCondition(lcKey, new RegExp(`^${escapeRegExp(val)}`));
    } else if (operator === 'ei') {
      addCondition(lcKey, new RegExp(`${escapeRegExp(val)}$`, 'i'));
    } else if (operator === 'bi') {
      addCondition(lcKey, new RegExp(`^${escapeRegExp(val)}`, 'i'));
    } else if (operator === 'm') {
      // key={m}<key>,<value>
      ...
      const tmp = val.match(/(.*)\/(.*)/);
      if (tmp) {
        const regexp = new RegExp(escapeRegExp(tmp[1]), escapeRegExp(tmp[2]));
        addCondition(lcKey, regexp);
      } else {
  ...
  for (const key in query) {
    ...
    if (key === 'q') {
      qy.q = _.isString(value) ? JSON.parse(value) : value;
    ...
    } else if (someOf('to', 'timeout')) {
      qy.options.maxTimeMS = parseInt(value, 10);
    } else if (someOf('x', 'explain')) {
      const allowedExplains = _.reduce(['queryPlanner', 'executionStats', 'allPlansExecution'],
        (acc, explain) => {
          acc[explain] = explain;
          return acc;
        }, {});
      const explain = _.get(allowedExplains, value, 'queryPlanner');
      qy.options.explain = explain;
    } else if (someOf('d', 'allowDiskUse')) {
      qy.options.allowDiskUse = toBool(value);
    } else if (someOf('p')) {
